...
const sessionEntryStore = new Map<string, Record<string, unknown>>();
...
vi.mock("../config/sessions.js", () => ({
  ...
  getSessionEntry: ({ sessionKey }: { sessionKey: string }) => sessionEntryStore.get(sessionKey),
  listSessionEntries: () =>
    [...sessionEntryStore.entries()].map(([sessionKey, entry]) => ({ sessionKey, entry })),
  upsertSessionEntry: ({
    sessionKey,
    entry,
  }: {
    ...
    sessionEntryStore.set(sessionKey, entry);
  },
}));
...
vi.mock("../gateway/session-utils.js", () => ({
  ...
  loadCombinedSessionEntriesForGateway: () => ({
    ...
    entries: Object.fromEntries(sessionEntryStore),
  }),
...
describe("EmbeddedTuiBackend", () => {
  ...
  beforeEach(() => {
    ...
    sessionEntryStore.clear();
    sessionEntryStore.set("agent:main:main", { sessionId: "existing-session" });
